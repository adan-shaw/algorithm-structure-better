...
{
	if (p_info == NULL || buff == NULL)
		return enum_list_xzy_err_param_NULL;
	if (unit_size < 1)
		return enum_list_xzy_err_param_wrong;
	if (size_buff < unit_size)
		return enum_list_xzy_err_param_small;
...
ENUM_LIST_XZY_ERR list_xzy_start (volatile STRUCT_LIST_XZY * p_info)
{
	if (p_info == NULL)
		return enum_list_xzy_err_param_NULL;
	if (p_info->flag_on_off != enum_rw_step_work)
		return enum_list_xzy_err_param_wrong;
...
ENUM_LIST_XZY_ERR list_xzy_read (volatile STRUCT_LIST_XZY * p_info, unsigned char *p_data, unsigned int size_data, ENUM_LIST_XZY_RW_STEP flag_updata)
{
	...
	if (p_data == NULL)
		return enum_list_xzy_err_param_NULL;
	if (size_data < p_info->unit_size)
		return enum_list_xzy_err_param_small;

	if (p_info->r.flag_on_off != enum_rw_step_work)
		return enum_list_xzy_err_read_off;
	if (p_info->r.step == enum_rw_step_work)
		return enum_list_xzy_err_read_busy;
	...
	if (p_info->flag_cover == enum_rw_step_work)
	{
		if (p_info->r.cir_times == p_info->w.cir_times)
			;
		else if ((p_info->r.cir_times + 1) == p_info->w.cir_times && p_info->r.index >= p_info->w.index)
			;
		else
		{
	...
	if (p_info->r.cir_times == p_info->w.cir_times && p_info->r.index == p_info->w.index)
		return enum_list_xzy_err_idle;
	memcpy (p_data, p_info->cache_address + p_info->r.index, p_info->unit_size);
	tmp_info_1 = *p_info;
	if (p_info->flag_cover == enum_rw_step_work)
	{
		if (tmp_info.w.cir_times == tmp_info_1.w.cir_times && tmp_info.w.index == tmp_info_1.w.index)
			;
		if (p_info->r.index == tmp_info_1.w.index && tmp_info_1.w.step == enum_rw_step_work)
		{
			return enum_list_xzy_err_recover;
		}
		else if (p_info->r.cir_times == tmp_info.w.cir_times && p_info->r.index < tmp_info.w.index && p_info->r.cir_times == tmp_info_1.w.cir_times && p_info->r.index < tmp_info_1.w.index)

			;
		else if (p_info->r.cir_times == tmp_info.w.cir_times && p_info->r.index < tmp_info.w.index && (p_info->r.cir_times + 1) == tmp_info_1.w.cir_times && p_info->r.index >= tmp_info_1.w.index)

			;
		else if ((p_info->r.cir_times + 1) == tmp_info.w.cir_times && p_info->r.index >= tmp_info.w.index && (p_info->r.cir_times + 1) == tmp_info_1.w.cir_times && p_info->r.index >= tmp_info_1.w.index)
...
ENUM_LIST_XZY_ERR list_xzy_get_num (volatile STRUCT_LIST_XZY * p_info, unsigned int *item_num)
{
	...
	if (p_info->r.flag_on_off != enum_rw_step_work)
		return enum_list_xzy_err_param_wrong;
	if (p_info->w.flag_on_off != enum_rw_step_work)
		return enum_list_xzy_err_param_wrong;
	if (item_num == NULL)
		return enum_list_xzy_err_param_NULL;
...
ENUM_LIST_XZY_ERR list_xzy_write (volatile STRUCT_LIST_XZY * p_info, unsigned char *p_data, unsigned int write_bytes)
{
	if (p_info == NULL)
		return enum_list_xzy_err_param_NULL;
	if (write_bytes > p_info->unit_size)
		return enum_list_xzy_err_param_wrong;
	if (p_info->flag_on_off != enum_rw_step_work)
		return enum_list_xzy_err_param_wrong;
	if (p_info->w.flag_on_off != enum_rw_step_work)
		return enum_list_xzy_err_param_wrong;
	if (p_info->w.step != enum_rw_step_work)
		return enum_list_xzy_err_write_busy;

	p_info->w.step = enum_rw_step_work;

	if (p_info->flag_cover == enum_rw_step_idle)
	{
		if ((p_info->r.cir_times + 1) == p_info->w.cir_times && p_info->w.index == p_info->r.index)
			return enum_list_xzy_err_recover;
	}
