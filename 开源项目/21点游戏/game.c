...
enum thetest
{ OK = 0, NO = 1, QUIT = 2, AGAIN = 3 };
enum kind
{ fp = 0, mh = 1, heit = 2, hongt = 3 };
struct feature
{
	int num;
	enum kind kin;
} card[52];//写牌
const char *suit (struct feature i)
{//记录扑克牌的花色
	if (i.kin == 0)
		return "方片";
	else if (i.kin == 1)
...
void playingcards (struct feature card[])
{//初始化牌库
	for (int i = 0; i <= 3; i++)
	{
		for (int n = 1; n <= 13; n++)
...
int main (void)
{
	...
	{//设置是否重新开始游戏, 游戏正式开始, 程序大框架
		...
		{//回合开始, 实现两人轮流抽牌
			printf ("\n请玩家%d选择是否抽一张牌,y/n/q\n", i);
			while (tip == 0)
			{//当tip=0意味本次游戏继续进行, tip=1是本次游戏结束的标识
