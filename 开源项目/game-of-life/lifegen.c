...
static unsigned cell_width (unsigned x)
{
	unsigned result = 1;
	unsigned value = 10;
	while (value <= x)
...
static void cell_name (char *dst, const struct config *c, unsigned x, unsigned y)
{
...
static bool rule_eval (const char *rules, unsigned count, bool state)
{
...
static void print_target (const struct config *c)
{
	printf ("output :\n");
...
static void print_rules (const struct config *c)
{
...
static void print_init (const struct config *c)
{
...
static void print_compute (const struct config *c)
{
	for (unsigned y = 0; y < c->height; y++)
		for (unsigned x = 0; x < c->width; x++)
		{
...
static void print_output (const struct config *c)
{
...
static void print_args (const struct config *c)
{
	printf ("args =");
	for (unsigned y = 0; y < c->height; y++)
		for (unsigned x = 0; x < c->width; x++)
...
static void print_help (FILE * o, const char *progname)
{
