	...
	default://彩蛋
		{
			...
			{
			case 1:
				printf ("(你为什么不按照套路来?不过, 我还是再给你一次机会吧)\n\n");
				goto a1;
				break;
			case 2:
				printf ("(你确定要这样吗?我以为你不讨厌玩游戏的)\n\n");
				goto a1;
				break;
			default:
				printf ("看来你很讨厌这个游戏\n");
...
a2:
	...
	case 0:
		...
		break;
	case 1:
		printf ("你因为太过胆怯而一直被困在这个屋子直到死去; (你也太胆小了)\n");
		system ("pause");
		printf ("END1: 缺乏勇气的冒险者; \n");
		system ("pause");
		exit (0);
...
void part9 (char name[], char gender[], int s)
{
	...
	system ("pause");
	printf ("" � ��终于归来了, 我的朋友; ",%s眼前的陌生人满脸欣喜的迎接着他; \n", name);
	system ("pause");
	system ("cls");

	printf ("%s现在只觉得心中紧绷着的弦一下子放松, 算了, 怎么样都好; \n我终于离开了那个鬼地方; \n", name);
	system ("pause");
	printf ("眼前的青年人迫切地向%s伸出双手: " � ��一个自由意志, %s, 欢迎你的回来; \n � ��在你可能搞不懂很多东西, 但是不要紧, 回归我们的世界, 你就会慢慢恢复记忆了; "\n", name, name);
	system ("pause");
	printf ("" � ��过, %s, 我们得首先给你一个编号; "\n", name);
	system ("pause");
	system ("cls");
...
void part10 (char name[], char gender[], int s)
{
	...
	a6:printf ("%s犹豫了起来, %s下定决心?(需要存档请输入0)\n1.询问\"我\"\n2.自己摸索这个屋子\n", name, gender);
		...
		case 0:
			...
			break;
		case 1:
			printf ("(你要知道, 我这个人, 其实很友好的; )\n");
			system ("pause");
			part12 (name, gender, 12);
			break;
		case 2:
			printf ("(嗯哼, 居然不相信我, 有趣; )\n");
			system ("pause");
			part13 (name, gender, 13);
			break;
		default:
			goto a6;
			break;
