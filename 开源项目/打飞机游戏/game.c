...
void init (map * start, setting * dat)//初始化数据
{
	...
	gotoxy (WIDES, COLS / 2 - 2);
	printf ("游戏状态: ");
	gotoxy (WIDES + 2, COLS / 2);
	printf ("开始!");
	gotoxy (WIDES, COLS / 2 + 3);
	printf ("操作方法: ");
	gotoxy (WIDES + 1, COLS / 2 + 6);
	printf ("方向键操作战机");
	gotoxy (WIDES + 1, COLS / 2 + 7);
	printf ("空格开火");
	gotoxy (WIDES + 1, COLS / 2 + 8);
	printf ("F2暂停");
	for (i = 0; i < WIDES; i++)
		for (j = 0; j < COLS; j++)
		{
...
//if (speed == 1000)speed = 0;
	if (tick (dat->MoveSpeed))
	{
		...
		if (GetAsyncKeyState (VK_UP) && dat->Pos[0] != 1)
			dat->Pos[0]--;
		if (GetAsyncKeyState (VK_LEFT) && dat->Pos[1] != 1)
			dat->Pos[1]--;
		if (GetAsyncKeyState (VK_DOWN) && dat->Pos[0] != COLS - 2)
			dat->Pos[0]++;
		if (GetAsyncKeyState (VK_RIGHT) && dat->Pos[1] != WIDES - 2)
			dat->Pos[1]++;
		if (GetAsyncKeyState (VK_SPACE))
			dat->AmmoPos[dat->Pos[0] - 1][dat->Pos[1]] = 2;
		start->MapSize[dat->Pos[0]][dat->Pos[1]] = 1;
	}
}
