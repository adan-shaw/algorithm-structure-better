...
int is_win ()//  是否结束游戏
{
	if (chess_in[x + x_offset][y + y_offset] == 'j' || chess_in[x + x_offset][y + y_offset] == 'k')
	{
...
int name_xiang ()//  是否象相的规则处理
{
	if (((x_offset == 2 && y_offset == -2 && chess_in[x + 1][y - 1] == 'a')//别象脚
			|| (x_offset == -2 && y_offset == 2 && chess_in[x - 1][y + 1] == 'a') || (x_offset == -2 && y_offset == -2 && chess_in[x - 1][y - 1] == 'a') || (x_offset == 2 && y_offset == 2 && chess_in[x + 1][y + 1] == 'a')) && ((x <= 4 && x + x_offset <= 4)//不能过河
			|| (x >= 5 && x + x_offset >= 5)))
		go ();
	return 0;
}
...
int name_jiang ()// 是否将帅的规则处理
{
	...
	if (((((x_offset == 1 || x_offset == -1) && (y_offset == 0))//走一格
				|| ((y_offset == 1 || y_offset == -1) && (x_offset == 0))) && ((x <= 2 && x + x_offset <= 2) || (x >= 7 && x + x_offset >= 7))//x.area
			&& ((y >= 3 && y <= 5) && (y + y_offset >= 3 && y + y_offset <= 5)))//y.area
