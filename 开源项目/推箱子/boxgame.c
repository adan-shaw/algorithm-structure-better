...
void Map_Build ()//随机生成地图函数
{
	...
	while ((People.x == Box.x && People.y == Box.y) || (People.x == Destination.x && People.y == Destination.y) || (Box.x == Destination.x && Box.y == Destination.y));//如果三个坐标其中有两个坐标相等的话就重新生成
