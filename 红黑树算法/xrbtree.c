...
static xrbt_void_t *xrbt_comm_node_memalloc (xrbt_vkey_t xrbt_vkey, xrbt_size_t xst_nsize, xrbt_ctxt_t xrbt_ctxt)
{
	return xrbt_heap_alloc (xst_nsize);
}

static xrbt_void_t xrbt_comm_node_memfree (x_rbnode_iter xiter_node, xrbt_size_t xnode_size, xrbt_ctxt_t xrbt_ctxt)
{
	xrbt_heap_free (xiter_node);
}

static xrbt_void_t xrbt_comm_vkey_copyfrom (xrbt_vkey_t xrbt_dkey, xrbt_vkey_t xrbt_skey, xrbt_size_t xrbt_size, xrbt_bool_t xbt_move, xrbt_ctxt_t xrbt_ctxt)
{
	memcpy (xrbt_dkey, xrbt_skey, xrbt_size);
}

static xrbt_void_t xrbt_comm_vkey_destruct (xrbt_vkey_t xrbt_vkey, xrbt_size_t xrbt_size, xrbt_ctxt_t xrbt_ctxt)
{

}

static xrbt_bool_t xrbt_comm_vkey_compare (xrbt_vkey_t xrbt_lkey, xrbt_vkey_t xrbt_rkey, xrbt_size_t xrbt_size, xrbt_ctxt_t xrbt_ctxt)
{
...
static x_rbnode_iter xrbtree_far_left (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
...
static x_rbnode_iter xrbtree_far_right (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
...
static xrbt_void_t xrbtree_dealloc (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
	XASSERT (XNODE_NOT_NIL (xiter_node));

	xthis_ptr->xcallback.xfunc_k_destruct (XNODE_VKEY (xiter_node), xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback);

	xthis_ptr->xcallback.xfunc_n_memfree (xiter_node, sizeof (x_rbtree_node_t) + xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback);
}

static xrbt_void_t xrbtree_clear_branch (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_branch_root)
{
#if 1
...
static x_rbnode_iter xrbtree_successor (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
	...
	while (XNODE_NOT_NIL (xiter_parent) && (xiter_parent->xiter_right == xiter_node))
	{
...
static x_rbnode_iter xrbtree_precursor (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
	x_rbnode_iter xiter_parent = xiter_node->xiter_parent;
	...
	while (XNODE_NOT_NIL (xiter_parent) && (xiter_parent->xiter_left == xiter_node))
	{
		xiter_node = xiter_parent;
		xiter_parent = xiter_parent->xiter_parent;
	}
...
static xrbt_void_t xrbtree_left_rotate (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
...
static xrbt_void_t xrbtree_right_rotate (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_node)
{
...
static xrbt_void_t xrbtree_dock_fixup (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_where)
{
...
static xrbt_void_t xrbtree_undock_fixup (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_where, x_rbnode_iter xiter_parent)
{
	x_rbnode_iter xiter_sibling = XTREE_GET_NIL (xthis_ptr);

	for (; (xiter_where != xthis_ptr->xiter_root) && (X_BLACK == xiter_where->xut_color); xiter_parent = xiter_where->xiter_parent)
	{
		if (xiter_where == xiter_parent->xiter_left)
		{
			...
			else if ((X_BLACK == xiter_sibling->xiter_left->xut_color) && (X_BLACK == xiter_sibling->xiter_right->xut_color))
			{
		...
		else
		{
			...
			else if ((X_BLACK == xiter_sibling->xiter_right->xut_color) && (X_BLACK == xiter_sibling->xiter_left->xut_color))
			{
				xiter_sibling->xut_color = X_RED;
				xiter_where = xiter_parent;
			}
...
static xrbt_void_t xrbtree_update (x_rbtree_ptr xthis_ptr, x_rbnode_iter xiter_where, xrbt_bool_t xbt_erase)
{
	if (xbt_erase)
	{
		...
		{
			xthis_ptr->xiter_lnode = xrbtree_far_left (xthis_ptr, xthis_ptr->xiter_root);
		}

		if (xthis_ptr->xiter_rnode == xiter_where)
		{
			xthis_ptr->xiter_rnode = xrbtree_far_right (xthis_ptr, xthis_ptr->xiter_root);
		}
	}
	else
	{
		...
		if (XNODE_IS_NIL (xthis_ptr->xiter_lnode) || xthis_ptr->xcallback.xfunc_k_compare (XNODE_VKEY (xiter_where), XNODE_VKEY (xthis_ptr->xiter_lnode), xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback))
		{
			xthis_ptr->xiter_lnode = xiter_where;
		}

		if (XNODE_IS_NIL (xthis_ptr->xiter_rnode) || xthis_ptr->xcallback.xfunc_k_compare (XNODE_VKEY (xthis_ptr->xiter_rnode), XNODE_VKEY (xiter_where), xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback))
		{
...
static x_rbnode_iter xrbtree_dock_pos (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey, xrbt_int32_t * xit_select)
{
	...
	while (XNODE_NOT_NIL (xiter_ntrav))
	{
		...
		xbt_to_left = xthis_ptr->xcallback.xfunc_k_compare (xrbt_vkey, XNODE_VKEY (xiter_ntrav), xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback);
	...
	if (xthis_ptr->xcallback.xfunc_k_compare (XNODE_VKEY (xiter_ntrav), xrbt_vkey, xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback))
	{
		return xiter_where;
	}
...
static x_rbnode_iter xrbtree_insert_nkey (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey, xrbt_bool_t xbt_move, xrbt_bool_t * xbt_ok)
{
	...
	xiter_node = (x_rbnode_iter) xthis_ptr->xcallback.xfunc_n_memalloc (xrbt_vkey, sizeof (x_rbtree_node_t) + xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback);
	XASSERT (XRBT_NULL != xiter_node);

	xthis_ptr->xcallback.xfunc_k_copyfrom (XNODE_VKEY (xiter_node), xrbt_vkey, xthis_ptr->xst_ksize, xbt_move, xthis_ptr->xcallback.xctxt_t_callback);
...
-å¤±è´¥, è¿”å›žXRBT_NULL ï ¿½ï¿½ï¿½*/x_rbtree_ptr xrbtree_create (xrbt_size_t xst_ksize, xrbt_callback_t * xcallback)
{
...
x_rbtree_ptr xrbtree_emplace_create (x_rbtree_ptr xthis_ptr, xrbt_size_t xst_ksize, xrbt_callback_t * xcallback)
{
	XASSERT (XRBT_NULL != xthis_ptr);
	XASSERT ((xst_ksize > 0) && (xst_ksize <= 0x7FFFFFFF));
...
#define XFUC_CHECK_SET(xfunc, xcheck, xdef) \
	...
	{
		XFUC_CHECK_SET (xthis_ptr->xcallback.xfunc_n_memalloc, xcallback->xfunc_n_memalloc, &xrbt_comm_node_memalloc);
		XFUC_CHECK_SET (xthis_ptr->xcallback.xfunc_n_memfree, xcallback->xfunc_n_memfree, &xrbt_comm_node_memfree);
		XFUC_CHECK_SET (xthis_ptr->xcallback.xfunc_k_copyfrom, xcallback->xfunc_k_copyfrom, &xrbt_comm_vkey_copyfrom);
		XFUC_CHECK_SET (xthis_ptr->xcallback.xfunc_k_destruct, xcallback->xfunc_k_destruct, &xrbt_comm_vkey_destruct);
		XFUC_CHECK_SET (xthis_ptr->xcallback.xfunc_k_compare, xcallback->xfunc_k_compare, &xrbt_comm_vkey_compare);
...
x_rbnode_iter xrbtree_insert (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey, xrbt_bool_t * xbt_ok)
{
	XASSERT (XRBT_NULL != xthis_ptr);
...
x_rbnode_iter xrbtree_insert_mkey (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey, xrbt_bool_t * xbt_ok)
{
	XASSERT (XRBT_NULL != xthis_ptr);
	XASSERT (XRBT_NULL != xrbt_vkey);
...
x_rbnode_iter xrbtree_find (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey)
{
	...
	if (XNODE_IS_NIL (xiter_node) || xthis_ptr->xcallback.xfunc_k_compare (xrbt_vkey, XNODE_VKEY (xiter_node), xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback))
	{
...
x_rbnode_iter xrbtree_lower_bound (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey)
{
	...
	{
		if (xthis_ptr->xcallback.xfunc_k_compare (XNODE_VKEY (xiter_trav), xrbt_vkey, xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback))
		{
...
x_rbnode_iter xrbtree_upper_bound (x_rbtree_ptr xthis_ptr, xrbt_vkey_t xrbt_vkey)
{
	...
	while (XNODE_NOT_NIL (xiter_trav))
	{
		if (xthis_ptr->xcallback.xfunc_k_compare (xrbt_vkey, XNODE_VKEY (xiter_trav), xthis_ptr->xst_ksize, xthis_ptr->xcallback.xctxt_t_callback))
		{
			xiter_node = xiter_trav;
			xiter_trav = xiter_trav->xiter_left;
		}
